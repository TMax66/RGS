...
confHSA <- reactive({
  conf %>%
    filter(anno == input$selanno, settore == "Sanità Animale")
  })
...
confHAU <- reactive({
  conf %>% filter(anno == input$selanno, settore == "Alimenti Uomo")
  })
...
confHAZ <- reactive({
  conf %>% filter(anno == input$selanno, settore == "Alimenti Zootecnici")
  })
