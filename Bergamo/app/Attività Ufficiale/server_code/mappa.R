...
output$bgbuffmap <- renderLeaflet({
  
  leaflet() %>% 
    addTiles() %>%
    setView(9.8, 45.75, zoom = 9) %>% 
    ...
    addPolygons(data = combg, stroke = TRUE, fill =  FALSE, weight = 1.5, color = "gray")
...
foc <- reactive(focolaio() %>% dplyr::select(long, lat) %>% na.omit())
...
observeEvent( #inserito il codice lo mappa sulla cartina con un marker
  input$codaz2,
  {
    leafletProxy("bgbuffmap",session, data = focolaio() ) %>%
      addMarkers(~long,  ~lat) #%>%
...
observeEvent( #cliccando genera mappa produce il buffer con gli allevamenti entro il raggio indicato e produce la tabella
  input$prot,
  {
    ...
    if(input$codaz2 %in% coordbv$codaz)
    {
      ...
      leafletProxy("bgbuffmap", session, data = entro()) %>%
  ...
  input$unprot,
   {
    ...
    leafletProxy("bgbuffmap", session, data = allcod()) %>%
      clearShapes() %>%
      clearMarkers() %>%
      setView(9.8, 45.75, zoom = 9) %>% 
      addPolygons(data = LOM, fill = FALSE, weight = 1, color = "black") %>%
...
#Codici per generare mappe con allevamenti campionati----
...
observeEvent( #cliccando genera mappa produce la mappa con gli allevamenti controllati per il campionamento selezionato
  input$prot2,
  {
    ...
    removeUI("#mappacamp") #cancella la tabella ...    
      ...
      leafletProxy("bgbuffmap", session, data = campmappa()) %>%
        clearMarkers() %>% clearShapes() %>%
        setView(9.8, 45.75, zoom = 9) %>% 
        addPolygons(data = LOM, fill = FALSE, weight = 1, color = "black") %>%
        addPolygons(data = PRlom, fill = FALSE, weight = 1, color = "black") %>%
