...
#   confSicA %>% filter(ASL == select_aslalim, finalita == input$finalita2, anno == input$anno2) %>%
#     left_join(proveSicA, by = c("nconf")) %>% 
#     dplyr::select( nconf, prova, tecnica, dtinizio, dtfine, numero_del_campione, "esito" = valore)
#   
# })
# 
# 
      ...
      options = list(
        ...
        buttons = list(
          list(extend = "excel", text = "Scarica Tutto",
               filename = paste0(input$finalita2, " - attività ufficiale ", input$anno2, " (dati aggiornati al ", format(as.Date(substr(max(conf$dtreg, na.rm = TRUE), start = 1, stop = 11)), "%d-%m-%Y"), ")"),
               title = NULL,
               titleAttr = "Excel",
               exportOptions = list(
        ...
        columnDefs = list(
          ...
          list(width = '10px', targets =c(1, 11)),
