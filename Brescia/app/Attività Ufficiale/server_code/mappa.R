...
output$BSbuffmap <- renderLeaflet({
  ...
  leaflet() %>% 
    ...
    addPolygons(data = BS, stroke = TRUE, fill =  FALSE, weight = 2.5, color = "black") %>%
    addPolygons(data = comBS, stroke = TRUE, fill =  FALSE, weight = 1.5, color = "gray") 
...
observeEvent( #inserito il codice lo mappa sulla cartina con un marker
  input$codaz2,
  {
    leafletProxy("BSbuffmap",session, data = focolaio() ) %>%
      addMarkers(~long,  ~lat) #%>%
...
observeEvent( #cliccando genera mappa produce il buffer con gli allevamenti entro il raggio indicato e produce la tabella
  input$prot,
  {
    ...
    if(input$codaz2 %in% coordbv$codaz)
    {
      ...
      leafletProxy("BSbuffmap", session, data = entro()) %>%
...
observeEvent(
  input$unprot,
  {
    ...
    leafletProxy("BSbuffmap", session, data = allcod()) %>%
      clearShapes() %>%
      clearMarkers() %>%
      setView(10.27, 45.78, zoom = 9) %>% 
      addPolygons(data = LOM, fill = FALSE, weight = 1, color = "black") %>% 
...
observeEvent( #cliccando genera mappa produce la mappa con gli allevamenti controllati per il campionamento selezionato
  input$prot2,
  {
    ...
    leafletProxy("BSbuffmap", session, data = campmappa()) %>%
      clearMarkers() %>% clearShapes() %>%
      setView(10.27, 45.78, zoom = 9) %>% 
      addPolygons(data = LOM, fill = FALSE, weight = 1, color = "black") %>% 
      addPolygons(data = PRlom, fill = FALSE, weight = 1, color = "black") %>%
