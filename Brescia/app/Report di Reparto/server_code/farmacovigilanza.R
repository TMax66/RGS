...
observeEvent(input$farma_bttn1, {
  shinyjs::show("farma_cp1")
  shinyjs::hide("farma_cp2")
  runjs('document.getElementById("table_farma").scrollIntoView();')
  })

observeEvent(input$farma_bttn2, {
  shinyjs::show("farma_cp2")
  shinyjs::hide("farma_cp1")
  runjs('document.getElementById("farma_esami").scrollIntoView();')
})


observe({
  mindate <- min(farmacovigilanza$dtinizio[farmacovigilanza$annoiniz == input$selanno], na.rm = TRUE)
  maxdate <- max(farmacovigilanza$dtinizio[farmacovigilanza$annoiniz == input$selanno], na.rm = TRUE)
...
# outputOptions(output, "farmaP1", suspendWhenHidden = FALSE)

##TEMPI MEDI PER PROVA----

output$farmaP2 <- renderPlotly({
  
  req(input$farma_prova)
  ...
  farmacovigilanza %>%
    ...
    mutate(
      ...
      intervallo = case_when(
        ...
        settiniz %in% 1:51 ~ paste("dal", format(sett_inizio, "%d/%m/%Y"),"al", format(sett_inizio + 6, "%d/%m/%Y")),
        settiniz == 0 ~ paste("dal", format(as.Date(paste(input$selanno, 1, 1, sep = "-"), "%Y-%m-%d"),"%d/%m/%Y"),"al", format(sett_inizio - 1, "%d/%m/%Y")))) %>% 
    filter(prova == input$farma_prova) %>% 
    plot_ly(x = ~settiniz,
            hoverinfo = 'text',
  ...
  tags$div(
    ...
    tags$h5(paste("(", n, " esami totali"," - dati aggiornati al ",
                  format(as.Date(substr(
                    max(farmacovigilanza$dtreg[farmacovigilanza$annoiniz == input$selanno], na.rm = TRUE), start = 1, stop = 11)), "%d-%m-%Y"),
                  ")", sep=""))
  )
})
...
output$farmaT1 <- renderDataTable({
  dt_farma() %>% 
    datatable(
      style = 'bootstrap',
      class = "row-border",
...
# outputOptions(output, "farmaT1", suspendWhenHidden = FALSE)

##VEDI CONFERIMENTI----
# output$table_farma <- renderUI({
...
output$table_farma <- renderDataTable(server = TRUE, {
  ...
  conf_farma() %>% 
    DT::datatable(
      ...
      options = list(
        ...
        #        filename = paste("Laboratorio di farmacovigilanza", "- dati al", format(as.Date(substr(max(farmacovigilanza$dtreg, na.rm = TRUE), start = 1, stop = 11)), "%d-%m-%Y")),
        #        title = NULL,
        #        titleAttr = "Excel",
        #        exportOptions = list(
...
###esami drilldown----
output$selconf_farma <- renderUI({
  
  sel_conf <- conf_farma()[as.integer(input$table_farma_rows_selected), ]$Nconf
...
output$drillui_farma <- renderUI({
  
  if (length(input$table_farma_rows_selected) > 0){
    
    div(style = "margin-bottom: 100px;",
        uiOutput("selconf_farma", style = "font-weight: 600;font-size: 18px;margin-top: 10px;margin-bottom: 10px;"),
        br(),
        dataTableOutput("drill_farma")
        )
    }
  })


prove_farma <- reactive({
  shiny::validate(
    need(length(input$table_farma_rows_selected) > 0, "")
  )
  
  select_conf <- conf_farma()[as.integer(input$table_farma_rows_selected), ]$Nconf
...
output$drill_farma <- DT::renderDataTable(server = TRUE,{
  prove_farma()
})
...
##VEDI ESAMI----
farma_esami <- reactive({
  
  if(input$farma_prove == "Tutte le prove"){
    
    farmacovigilanza %>%
      filter(annoiniz == input$selanno,
             # prova == input$esam,
             dtinizio >= input$dateRangeEsam_farma[1] & dtinizio <= input$dateRangeEsam_farma[2]) %>%
...
output$farma_esami <- DT::renderDataTable(server = TRUE,{
  
  if (input$doesami_farma == 0)
    return()
...
#DOWNLOAD----
output$farma_downloadConf <- downloadHandler(
  filename = function() {
    paste('Laboratorio di Farmacovigilanza e Antibioticoresistenza - conferimenti al ',
          format(as.Date(substr(max(farmacovigilanza$dtreg[farmacovigilanza$annoiniz == input$selanno], na.rm = TRUE), start = 1, stop = 11)), "%d-%m-%Y"),
          '.xlsx',
          sep='')
  },
...
output$farma_downloadEsam <- downloadHandler(
  filename = function() {
    paste('Laboratorio di Farmacovigilanza e Antibioticoresistenza - prove al ',
          format(as.Date(substr(max(farmacovigilanza$dtinizio[farmacovigilanza$annoiniz == input$selanno], na.rm = TRUE), start = 1, stop = 11)), "%d-%m-%Y"),
          '.xlsx',
          sep='')
  },
  content = function(con) {
