...
observeEvent(input$micro_bttn1, {
  shinyjs::show("micro_cp1")
  shinyjs::hide("micro_cp2")
  runjs('document.getElementById("table_micro").scrollIntoView();')
  })

observeEvent(input$micro_bttn2, {
  shinyjs::show("micro_cp2")
  shinyjs::hide("micro_cp1")
  runjs('document.getElementById("micro_esami").scrollIntoView();')
})


observe({
  mindate <- min(microbiologia$dtinizio[microbiologia$annoiniz == input$selanno], na.rm = TRUE)
  maxdate <- max(microbiologia$dtinizio[microbiologia$annoiniz == input$selanno], na.rm = TRUE)
...
# outputOptions(output, "microP1", suspendWhenHidden = FALSE)

##TEMPI MEDI PER PROVA----

output$microP2 <- renderPlotly({
  
  req(input$micro_prova)
  ...
  microbiologia %>%
    ...
    mutate(
      ...
      intervallo = case_when(
        ...
        settiniz %in% 1:51 ~ paste("dal", format(sett_inizio, "%d/%m/%Y"),"al", format(sett_inizio + 6, "%d/%m/%Y")),
        settiniz == 0 ~ paste("dal", format(as.Date(paste(input$selanno, 1, 1, sep = "-"), "%Y-%m-%d"),"%d/%m/%Y"),"al", format(sett_inizio - 1, "%d/%m/%Y")))) %>% 
    filter(prova == input$micro_prova) %>% 
    plot_ly(x = ~settiniz,
            hoverinfo = 'text',
  ...
  tags$div(
    ...
    tags$h5(paste("(", n, " esami totali"," - dati aggiornati al ",
                  format(as.Date(substr(
                    max(microbiologia$dtreg[microbiologia$annoiniz == input$selanno], na.rm = TRUE), start = 1, stop = 11)), "%d-%m-%Y"),
                  ")", sep=""))
  )
})
...
output$microT1 <- renderDataTable({
  dt_micro() %>% 
    datatable(
      style = 'bootstrap',
      class = "row-border",
...
# outputOptions(output, "microT1", suspendWhenHidden = FALSE)

##VEDI CONFERIMENTI----
# output$table_micro <- renderUI({
...
output$table_micro <- renderDataTable(server = TRUE, {
  ...
  conf_micro() %>% 
    DT::datatable(
      ...
      options = list(
        ...
        #        filename = paste("Laboratorio di microbiologia", "- dati al", format(as.Date(substr(max(microbiologia$dtreg, na.rm = TRUE), start = 1, stop = 11)), "%d-%m-%Y")),
        #        title = NULL,
        #        titleAttr = "Excel",
        #        exportOptions = list(
...
###esami drilldown----
output$selconf_micro <- renderUI({
  
  sel_conf <- conf_micro()[as.integer(input$table_micro_rows_selected), ]$Nconf
...
output$drillui_micro <- renderUI({
  
  if (length(input$table_micro_rows_selected) > 0){
    
    div(style = "margin-bottom: 100px;",
        uiOutput("selconf_micro", style = "font-weight: 600;font-size: 18px;margin-top: 10px;margin-bottom: 10px;"),
        br(),
        dataTableOutput("drill_micro")
        )
    }
  })


prove_micro <- reactive({
  shiny::validate(
    need(length(input$table_micro_rows_selected) > 0, "")
  )
  
  select_conf <- conf_micro()[as.integer(input$table_micro_rows_selected), ]$Nconf
...
output$drill_micro <- DT::renderDataTable(server = TRUE,{
  prove_micro()
})
...
##VEDI ESAMI----
micro_esami <- reactive({
  
  if(input$micro_prove == "Tutte le prove"){
    
    microbiologia %>%
      filter(annoiniz == input$selanno,
             # prova == input$esam,
             dtinizio >= input$dateRangeEsam_micro[1] & dtinizio <= input$dateRangeEsam_micro[2]) %>%
...
output$micro_esami <- DT::renderDataTable(server = TRUE,{
  
  if (input$doesami_micro == 0)
    return()
...
#DOWNLOAD----
output$micro_downloadConf <- downloadHandler(
  filename = function() {
    paste('Laboratorio di Microbiologia degli alimenti - conferimenti al ',
          format(as.Date(substr(max(microbiologia$dtreg[microbiologia$annoiniz == input$selanno], na.rm = TRUE), start = 1, stop = 11)), "%d-%m-%Y"),
          '.xlsx',
          sep='')
  },
...
output$micro_downloadEsam <- downloadHandler(
  filename = function() {
    paste('Laboratorio di Microbiologia degli alimenti - prove al ',
          format(as.Date(substr(max(microbiologia$dtinizio[microbiologia$annoiniz == input$selanno], na.rm = TRUE), start = 1, stop = 11)), "%d-%m-%Y"),
          '.xlsx',
          sep='')
  },
  content = function(con) {
