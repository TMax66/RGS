#Biologia Molecolare e TSE----

observeEvent(input$biolo_bttn1, {
  shinyjs::show("biolo_cp1")
  shinyjs::hide("biolo_cp2")
  runjs('document.getElementById("table_biolo").scrollIntoView();')
  })

observeEvent(input$biolo_bttn2, {
  shinyjs::show("biolo_cp2")
  shinyjs::hide("biolo_cp1")
  runjs('document.getElementById("biolo_esami").scrollIntoView();')
})


observe({
  mindate <- min(biologia$dtinizio[biologia$annoiniz == input$selanno], na.rm = TRUE)
  maxdate <- max(biologia$dtinizio[biologia$annoiniz == input$selanno], na.rm = TRUE)
...
# outputOptions(output, "bioloP1", suspendWhenHidden = FALSE)

##TEMPI MEDI PER PROVA----

output$bioloP2 <- renderPlotly({
  
  req(input$biolo_prova)
  ...
  biologia %>%
    ...
    mutate(
      ...
      intervallo = case_when(
        ...
        settiniz %in% 1:51 ~ paste("dal", format(sett_inizio, "%d/%m/%Y"),"al", format(sett_inizio + 6, "%d/%m/%Y")),
        settiniz == 0 ~ paste("dal", format(as.Date(paste(input$selanno, 1, 1, sep = "-"), "%Y-%m-%d"),"%d/%m/%Y"),"al", format(sett_inizio - 1, "%d/%m/%Y")))) %>% 
    filter(prova == input$biolo_prova) %>% 
    plot_ly(x = ~settiniz,
            hoverinfo = 'text',
  ...
  tags$div(
    ...
    tags$h5(paste("(", n, " esami totali"," - dati aggiornati al ",
                  format(as.Date(substr(
                    max(biologia$dtreg[biologia$annoiniz == input$selanno], na.rm = TRUE), start = 1, stop = 11)), "%d-%m-%Y"),
                  ")", sep=""))
  )
})
...
output$bioloT1 <- renderDataTable({
  dt_biolo() %>% 
    datatable(
      style = 'bootstrap',
      class = "row-border",
...
# outputOptions(output, "bioloT1", suspendWhenHidden = FALSE)

##VEDI CONFERIMENTI----
# output$table_biolo <- renderUI({
...
output$table_biolo <- renderDataTable(server = TRUE, {
  ...
  conf_biolo() %>% 
    DT::datatable(
      ...
      options = list(
        ...
        #        filename = paste("Laboratorio di biologia", "- dati al", format(as.Date(substr(max(biologia$dtreg, na.rm = TRUE), start = 1, stop = 11)), "%d-%m-%Y")),
        #        title = NULL,
        #        titleAttr = "Excel",
        #        exportOptions = list(
...
###esami drilldown----
output$selconf_biolo <- renderUI({
  
  sel_conf <- conf_biolo()[as.integer(input$table_biolo_rows_selected), ]$Nconf
...
output$drillui_biolo <- renderUI({
  
  if (length(input$table_biolo_rows_selected) > 0){
    
    div(style = "margin-bottom: 100px;",
        uiOutput("selconf_biolo", style = "font-weight: 600;font-size: 18px;margin-top: 10px;margin-bottom: 10px;"),
        br(),
        dataTableOutput("drill_biolo")
        )
    }
  })


prove_biolo <- reactive({
  shiny::validate(
    need(length(input$table_biolo_rows_selected) > 0, "")
  )
  
  select_conf <- conf_biolo()[as.integer(input$table_biolo_rows_selected), ]$Nconf
...
output$drill_biolo <- DT::renderDataTable(server = TRUE,{
  prove_biolo()
})
...
##VEDI ESAMI----
biolo_esami <- reactive({
  
  if(input$biolo_prove == "Tutte le prove"){
    
    biologia %>%
      filter(annoiniz == input$selanno,
             # prova == input$esam,
             dtinizio >= input$dateRangeEsam_biolo[1] & dtinizio <= input$dateRangeEsam_biolo[2]) %>%
...
output$biolo_esami <- DT::renderDataTable(server = TRUE,{
  
  if (input$doesami_biolo == 0)
    return()
...
#DOWNLOAD----
output$biolo_downloadConf <- downloadHandler(
  filename = function() {
    paste('Laboratorio di Biologia Molecolare e TSE - conferimenti al ',
          format(as.Date(substr(max(biologia$dtreg[biologia$annoiniz == input$selanno], na.rm = TRUE), start = 1, stop = 11)), "%d-%m-%Y"),
          '.xlsx',
          sep='')
  },
...
output$biolo_downloadEsam <- downloadHandler(
  filename = function() {
    paste('Laboratorio di Biologia Molecolare e TSE - prove al ',
          format(as.Date(substr(max(biologia$dtinizio[biologia$annoiniz == input$selanno], na.rm = TRUE), start = 1, stop = 11)), "%d-%m-%Y"),
          '.xlsx',
          sep='')
  },
  content = function(con) {
