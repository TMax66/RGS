...
output$buffmap <- renderLeaflet({
  ...
  leaflet(
    ...
    addEasyButton(easyButton(
      ...
      onClick = JS("function(btn, map){ map.setView([44.54, 10.89], 9); }")))
...
all <- reactive({
  coordbv %>%
    dplyr::select(codaz, COMUNE,# XT_DENOMINAZIONE,
                  long, lat) %>%
    na.omit()
  })

protezione <- reactive({
  pointDistance(foc(), all()[, 3:4], lonlat = TRUE)<=input$area
  })
  ...
  input$prot,
  {
    ...
    if(input$codaz2 %in% coordbv$codaz)
    {
      ...
      insertUI("#placeholder", "afterEnd",
               ui = div(id = "allfoc",
                        wellPanel(style="width: 700px;",
                          h4("Elenco Aziende selezionate",style="text-align: center; font-weight: 600; margin-bottom: 20px;"),                                   
                 entro() %>% 
                 #left_join(coordbv, by = "codaz") %>%
                 select(codaz, # XT_DENOMINAZIONE,
                        COMUNE) %>% #, `Tipologia Allevamento`, "Indirizzo" = `Indirizzo sede Allev`) %>% 
